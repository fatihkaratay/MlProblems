...
class MultinomialNB:
    ...
    def train(self):
        ...
        self.likelihood_per_word_per_tag = self.__get_word_likelihoos_per_tag()
    ...
    def __get_word_likelihoods_per_tag(self):
        ...
        for word, tags_map in word_frequencies_per_tag.items():
            for tag in tags_map.keys():
                word_likelihoods_per_tag[word][tag] = (word_frequencies_per_tag[word][tag] + 1 * self.alpha) / (total_word_count_per_tag[tag] + 2 * self.alpha)
